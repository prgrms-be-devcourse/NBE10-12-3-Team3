...
class ProdInitDataService(
    ...
    fun insertAll(
        ...
        val heavyUsers = mutableListOf<User>()
        for (i in 0..9) {
            heavyUsers.add(
                userRepository.save(
                    User(
            ...
            generalUsers.add(
                userRepository.save(
                    User(
                        ...
                        pw,
                        GENERAL_NICKNAMES[i],
                        GENERAL_INTROS[i % GENERAL_INTROS.size],
                        UserRole.USER,
                    ),
                ),
            )
        ...
        val generalSeries = mutableListOf<Series>()
        for (i in 0..39) {
            generalSeries.add(
                seriesRepository.save(
                    Series(
                        generalUsers[i],
                        SERIES_TITLES[60 + i],
                        SERIES_BODIES[(60 + i) % SERIES_BODIES.size],
                    ),
                ),
            )
        }
        ...
        for (i in 0..9) {
            ...
            for (j in 0..39) {
                val series = if (j < 32) mySeries[j % 6] else null
                ...
                val post =
                    postRepository.save(
                        Post(
                            heavyUsers[i],
                            series,
                            POST_TITLES[(i * 40 + j) % POST_TITLES.size],
        ...
        for (i in 0..89) {
            val series = if (i < 40 && RNG.nextBoolean()) generalSeries[i] else null
            val postCount = 2 + RNG.nextInt(2)
        ...
        for (subscriber in generalUsers) {
            ...
            repeat(creatorCount) {
                ...
                val isMembership = RNG.nextInt(10) < 3
                val startedAt =
                    if (isMembership) {
