...
class SeriesControllerE2ETest {
    ...
    private static final long NON_EXISTENT_SERIES_ID = 999_999_999L;
    ...
    private static final Path SERIES_UPLOAD_DIR = Path.of("build", "test-uploadsseries");
    ...
    static void cleanUpUploadedFiles() throws IOException {
        if (!Files.exists(SERIES_UPLOAD_DIR)) {
            return;
        }
        try (Stream<Path> walk = Files.walk(SERIES_UPLOAD_DIR)) {
            walk.filter(path -> !path.equals(SERIES_UPLOAD_DIR))
    ...
    private static ParameterizedTypeReference<ApiResponse<SeriesResponse>> seriesBody() {
    ...
    private RestTestClient.ResponseSpec createSeriesRequest(String accessToken, String title, String body) {
        return client.post().uri("/api/series")
                .header("Authorization", bearer(accessToken))
                .contentType(MediaType.APPLICATION_JSON)
                .body(new SeriesCreateRequest(title, body))
                .exchange();
    }

    /** 생성 성공을 전제로 시리즈 id를 돌려준다. 다른 API의 대상 시리즈를 만들 때 쓴다. */
    private Long createSeries(String accessToken, String title, String body) {
        ApiResponse<SeriesResponse> response = createSeriesRequest(accessToken, title, body)
                .expectStatus().isCreated()
                .expectBody(seriesBody())
                .returnResult()
                .getResponseBody();
        return response.data().id();
    }

    private RestTestClient.ResponseSpec getSeries(Object id) {
        return client.get().uri("/api/series/" + id).exchange();
    }

    private RestTestClient.ResponseSpec updateSeriesRequest(String accessToken, Object id, String title, String body) {
        return client.put().uri("/api/series/" + id)
                .header("Authorization", bearer(accessToken))
                .contentType(MediaType.APPLICATION_JSON)
                .body(new SeriesUpdateRequest(title, body))
                .exchange();
    }

    private RestTestClient.ResponseSpec deleteSeriesRequest(String accessToken, Object id) {
        return client.delete().uri("/api/series/" + id)
                .header("Authorization", bearer(accessToken))
                .exchange();
    }
    ...
    private static String uniqueSearchKeyword() {
        return "e2eSeriesSearch" + UUID.randomUUID().toString().replace("-", "").substring(0, 10);
    }
    ...
    private static ParameterizedTypeReference<ApiResponse<SliceResult<SeriesListResponse>>> seriesSliceBody() {
        return new ParameterizedTypeReference<>() {};
    }

    private static ParameterizedTypeReference<ApiResponse<PageResponse<SeriesListResponse>>> seriesPageBody() {
        return new ParameterizedTypeReference<>() {};
    }

    private RestTestClient.ResponseSpec getSeriesListRequest(String queryString) {
        return client.get().uri("/api/series" + queryString).exchange();
    }

    private RestTestClient.ResponseSpec searchSeriesRequest(String queryString) {
        return client.get().uri("/api/series/search" + queryString).exchange();
    }

    private RestTestClient.ResponseSpec getUserSeriesListRequest(Object userId, String queryString) {
        return client.get().uri("/api/series/users/" + userId + queryString).exchange();
    }

    private RestTestClient.ResponseSpec getMySeriesListRequest(String accessToken, String queryString) {
        return client.get().uri("/api/series/me" + queryString)
                .header("Authorization", bearer(accessToken))
                .exchange();
    }
    ...
    private RestTestClient.ResponseSpec uploadMediaRequest(String accessToken, Object seriesId, MultiValueMap<String, HttpEntity<?>> multipartBody) {
        return client.post().uri("/api/series/" + seriesId + "/medias")
                .header("Authorization", bearer(accessToken))
    ...
    private RestTestClient.ResponseSpec getMediaRequest(Object seriesId) {
        return client.get().uri("/api/series/" + seriesId + "/medias").exchange();
    }

    private RestTestClient.ResponseSpec deleteMediaRequest(String accessToken, Object seriesId) {
        return client.delete().uri("/api/series/" + seriesId + "/medias")
                .header("Authorization", bearer(accessToken))
                .exchange();
    }

    private static ParameterizedTypeReference<ApiResponse<SeriesMediaResponse>> seriesMediaBody() {
        return new ParameterizedTypeReference<>() {};
    }

    /** 업로드 성공을 전제로 응답을 돌려준다. 다른 API의 "썸네일이 이미 있는 시리즈" 사전 상태를 만들 때 쓴다. */
    private SeriesMediaResponse uploadSeriesMedia(String accessToken, Long seriesId, byte[] content, String filename) {
        return uploadMediaRequest(accessToken, seriesId, mediaFilePart(content, filename, MediaType.IMAGE_PNG))
                .expectStatus().isCreated()
                .expectBody(seriesMediaBody())
                .returnResult()
                .getResponseBody()
                .data();
    }
    ...
    private Path uploadedFilePath(String url) {
        return SERIES_UPLOAD_DIR.resolve(url.substring("series/".length()));
    }
    ...
    private static final List<String> POST_LIST_RESPONSE_KEYS = List.of(
            "id", "userId", "nickname", "seriesId", "title",
            "publishStatus", "accessLevel",
            "viewCount", "likeCount", "bookmarkCount",
    ...
    private static ParameterizedTypeReference<ApiResponse<List<PostListResponse>>> seriesPostsBody() {
        return new ParameterizedTypeReference<>() {};
    }
    ...
    private Long createPost(String accessToken, String title, String body,
                            PublishStatus publishStatus, PostAccessLevel accessLevel) {
    ...
    private Long createPost(String accessToken, String title) {
        return createPost(accessToken, title, title + " 본문", PublishStatus.PUBLIC, PostAccessLevel.FREE);
    }

    /** 시리즈에 편입된 포스트를 만든다. 두 API를 잇는 픽스처라 각 단계의 성공을 확인하고 넘어간다. */
    private Long createPostInSeries(String accessToken, Long seriesId, String title) {
        Long postId = createPost(accessToken, title);
        expectResultCode(addPostToSeriesRequest(accessToken, seriesId, postId), HttpStatus.OK, "200-1");
        return postId;
    }

    private RestTestClient.ResponseSpec deletePostRequest(String accessToken, Object postId) {
        return client.delete().uri("/api/posts/" + postId)
                .header("Authorization", bearer(accessToken))
                .exchange();
    }

    private RestTestClient.ResponseSpec getPostRequest(Object postId) {
        return client.get().uri("/api/posts/" + postId).exchange();
    }

    private RestTestClient.ResponseSpec getSeriesPostsRequest(Object seriesId) {
        return client.get().uri("/api/series/" + seriesId + "/posts").exchange();
    }

    private RestTestClient.ResponseSpec getSeriesPostsRequest(String accessToken, Object seriesId) {
        return client.get().uri("/api/series/" + seriesId + "/posts")
                .header("Authorization", bearer(accessToken))
                .exchange();
    }

    private RestTestClient.ResponseSpec addPostToSeriesRequest(String accessToken, Object seriesId, Object postId) {
        return client.post().uri("/api/series/" + seriesId + "/posts/" + postId)
                .header("Authorization", bearer(accessToken))
                .exchange();
    }

    private RestTestClient.ResponseSpec removePostFromSeriesRequest(String accessToken, Object seriesId, Object postId) {
        return client.delete().uri("/api/series/" + seriesId + "/posts/" + postId)
                .header("Authorization", bearer(accessToken))
                .exchange();
    }

    /** 시리즈에 담긴 포스트 id들. 순서는 보장되지 않으므로(Q3, known-issues #6) 집합으로만 쓴다. */
    private List<Long> seriesPostIds(Object seriesId) {
        return getSeriesPostsRequest(seriesId)
                .expectStatus().isOk()
                .expectBody(seriesPostsBody())
                .returnResult()
                .getResponseBody()
                .data()
                .stream()
    ...
    private JsonNode firstSeriesPostNode(RestTestClient.ResponseSpec response) {
        String raw = response
                .expectStatus().isOk()
                .expectBody(String.class)
                .returnResult()
                .getResponseBody();
        JsonNode data = new ObjectMapper().readTree(raw).get("data");
        assertThat(data.size()).isEqualTo(1);
        return data.get(0);
    }

    private void likePost(String accessToken, Long postId) {
        expectResultCode(
                client.post().uri("/api/posts/" + postId + "/likes")
                        .header("Authorization", bearer(accessToken))
                        .exchange(),
                HttpStatus.CREATED, "201-1");
    }

    private void bookmarkPost(String accessToken, Long postId) {
        expectResultCode(
    ...
    private SeriesListResponse userSeriesListEntry(Long userId, Long seriesId) {
        return getUserSeriesListRequest(userId, "")
                .expectStatus().isOk()
                .expectBody(seriesPageBody())
                .returnResult()
                .getResponseBody()
                .data()
                .content()
                .stream()
                .filter(series -> series.id().equals(seriesId))
                .findFirst()
                .orElseThrow(() -> new AssertionError("유저 시리즈 목록에 시리즈 " + seriesId + " 가 없다"));
    }
    ...
    class CreateSeries {
        ...
        void createSeries_success_returns201AndIsRetrievable() {
            ...
            createSeriesRequest(accessToken, "e2e 생성 시리즈", "e2e 생성 본문")
                    ...
                    .value(body -> {
                        ...
                        createdIdHolder[0] = body.data().id();
                    });

            // 응답만 그럴듯한 게 아니라 실제로 저장되었는지 후속 조회로 확인한다.
            getSeries(createdIdHolder[0])
                    .expectStatus().isOk()
                    .expectBody(seriesBody())
                    .value(body -> {
                        assertThat(body.resultCode()).isEqualTo("200-1");
                        assertThat(body.data().id()).isEqualTo(createdIdHolder[0]);
                        assertThat(body.data().title()).isEqualTo("e2e 생성 시리즈");
                        assertThat(body.data().body()).isEqualTo("e2e 생성 본문");
                        assertThat(body.data().nickname()).isEqualTo(nickname);
                    });
        ...
        void createSeries_nullBody_returns201WithNullBody() {
            ...
            getSeries(createdIdHolder[0])
                    .expectStatus().isOk()
                    .expectBody(seriesBody())
                    .value(body -> assertThat(body.data().body()).isNull());
        }

        @Test
        @DisplayName("4. title이 없으면 400-1과 \"제목은 필수입니다.\"를 반환한다")
        void createSeries_missingTitle_returns400_1() {
            String accessToken = newUserToken();
        ...
        void createSeries_blankTitle_returns400_1() {
            String accessToken = newUserToken();
        ...
        void createSeries_malformedJson_returns400_1WithParseErrorMessage() {
            String accessToken = newUserToken();
    ...
    class GetSeries {
        ...
        void getSeries_anonymous_returns200WithSeries() {
            String nickname = uniqueNickname();
            String accessToken = createUserAndGetAccessToken(client, uniqueEmail(), DEFAULT_PASSWORD, nickname);
            Long seriesId = createSeries(accessToken, "비로그인 조회 시리즈", "비로그인 조회 본문");
        ...
        void getSeries_otherUser_returns200() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "타인이 조회할 시리즈", "본문");

            String otherToken = newUserToken();

            client.get().uri("/api/series/" + seriesId)
        ...
        void getSeries_nonExistentId_returns404_5() {
            expectResultCode(getSeries(NON_EXISTENT_SERIES_ID), HttpStatus.NOT_FOUND, "404-5");
        }

        @Test
        @DisplayName("4. 삭제된 시리즈면 404-5를 반환한다")
        void getSeries_softDeletedSeries_returns404_5() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "삭제될 시리즈", "본문");
    ...
    class UpdateSeries {
        ...
        @Test
        @DisplayName("1. 미인증이면 401-1과 \"로그인 후 이용해주세요.\"를 반환한다")
        void updateSeries_unauthenticated_returns401_1() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "미인증 수정 대상", "본문");

            expectEntryPointUnauthorized(
                    client.put().uri("/api/series/" + seriesId)
                            .contentType(MediaType.APPLICATION_JSON)
                            .body(new SeriesUpdateRequest("수정된 제목", "수정된 본문"))
                            .exchange());
        }

        @Test
        @DisplayName("2. 소유자가 수정하면 200과 수정된 내용을 반환하고, GET /{id}에도 반영된다")
        void updateSeries_owner_returns200AndIsReflected() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "수정 전 제목", "수정 전 본문");
        ...
        void updateSeries_nullBody_clearsBody() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "본문 지울 시리즈", "지워질 본문");
        ...
        void updateSeries_otherUsersSeries_returns403_1() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "남의 시리즈", "본문");
            ...
            getSeries(seriesId)
                    .expectStatus().isOk()
                    .expectBody(seriesBody())
                    .value(body -> assertThat(body.data().title()).isEqualTo("남의 시리즈"));
        }

        @Test
        @DisplayName("5. 존재하지 않는 id면 404-5를 반환한다")
        void updateSeries_nonExistentId_returns404_5() {
            String accessToken = newUserToken();

            expectResultCode(
                    updateSeriesRequest(accessToken, NON_EXISTENT_SERIES_ID, "제목", "본문"),
                    HttpStatus.NOT_FOUND, "404-5");
        }

        @Test
        @DisplayName("6. 삭제된 시리즈면 404-5를 반환한다")
        void updateSeries_softDeletedSeries_returns404_5() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "삭제 후 수정할 시리즈", "본문");
        ...
        void updateSeries_blankTitle_returns400_1() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "공백 제목으로 바꿀 시리즈", "본문");
        ...
        void updateSeries_missingTitle_returns400_1() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "제목 없이 수정할 시리즈", "본문");
        ...
        void updateSeries_owner_updatesUpdatedAt() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "updatedAt 검증용 시리즈", "본문");

            SeriesResponse before = getSeries(seriesId)
                    .expectStatus().isOk()
                    .expectBody(seriesBody())
                    .returnResult()
                    .getResponseBody()
    ...
    class DeleteSeries {
        ...
        @Test
        @DisplayName("1. 미인증이면 401-1과 \"로그인 후 이용해주세요.\"를 반환한다")
        void deleteSeries_unauthenticated_returns401_1() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "미인증 삭제 대상", "본문");
        ...
        void deleteSeries_owner_returns200_andSeriesIsGone() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "삭제할 시리즈", "본문");
        ...
        void deleteSeries_otherUsersSeries_returns403_1() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "남의 삭제 대상 시리즈", "본문");
        ...
        void deleteSeries_nonExistentId_returns404_5() {
            String accessToken = newUserToken();
        ...
        void deleteSeries_alreadyDeleted_returns404_5() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "두 번 삭제할 시리즈", "본문");
        ...
        void deleteSeries_owner_leavesThumbnailOrphaned() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "썸네일 고아 확인용 시리즈", "본문");
            SeriesMediaResponse uploaded = uploadSeriesMedia(accessToken, seriesId, PNG_BYTES, "thumbnail.png");
            long mediaCountBeforeDelete = mediaRepository.count();
            ...
            Optional<SeriesMedia> seriesMediaAfterDelete = seriesMediaRepository.findBySeries(series);
            assertThat(seriesMediaAfterDelete).isPresent();
            assertThat(seriesMediaAfterDelete.get().getId()).isEqualTo(uploaded.id());
    ...
    class GetSeriesList {
        ...
        void getSeriesList_anonymous_returns200WithCreatedIdIncluded() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "전체 목록 포함 확인 시리즈", "본문");

            getSeriesListRequest("")
                    .expectStatus().isOk()
                    .expectBody(seriesSliceBody())
                    .value(body -> {
                        assertThat(body.resultCode()).isEqualTo("200-1");
                        assertThat(body.data().content())
                                .extracting(SeriesListResponse::id)
                                .contains(seriesId);
                    });
        }
        ...
        void getSeriesList_exposesRawSliceSchema() throws Exception {
            newUserToken();

            String raw = getSeriesListRequest("?size=1")
                    .expectStatus().isOk()
                    .expectBody(String.class)
                    .returnResult()
                    .getResponseBody();
        ...
        void getSeriesList_includesOtherUsersSeries() {
            String ownerAToken = newUserToken();
            Long seriesAId = createSeries(ownerAToken, "A의 전체목록 시리즈", "본문");
            String ownerBToken = newUserToken();
            Long seriesBId = createSeries(ownerBToken, "B의 전체목록 시리즈", "본문");
        ...
        void getSeriesList_respectsSizeAndPage() {
            String accessToken = newUserToken();
            Long olderId = createSeries(accessToken, "size page 이전 시리즈", "본문");
            Long newerId = createSeries(accessToken, "size page 이후 시리즈", "본문");
            ...
            getSeriesListRequest("?size=1&page=0")
                    ...
                    .value(body -> {
                        ...
                        assertThat(body.data().content()).extracting(SeriesListResponse::id).containsExactly(newerId);
                    });
            ...
            getSeriesListRequest("?size=1&page=1")
                    ...
                    .value(body -> {
                        ...
                        assertThat(body.data().content()).extracting(SeriesListResponse::id).containsExactly(olderId);
                    });
        }

        @Test
        @DisplayName("5. soft delete된 시리즈는 목록에서 제외된다")
        void getSeriesList_excludesSoftDeletedSeries() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "삭제 후 목록에서 빠질 시리즈", "본문");
            deleteSeriesRequest(accessToken, seriesId).expectStatus().isOk();
        ...
        void getSeriesList_invalidSortProperty_returns500_1() {
    ...
    class SearchSeries {
        ...
        void searchSeries_uniqueKeyword_returns200WithPageResponse() {
            String accessToken = newUserToken();
            String keyword = uniqueSearchKeyword();
            Long seriesId = createSeries(accessToken, keyword, "본문");
            ...
            searchSeriesRequest("?keyword=" + keyword)
                    ...
                    .value(body -> {
                        ...
                        assertThat(body.data().content()).extracting(SeriesListResponse::id).containsExactly(seriesId);
                    });
        }

        @Test
        @DisplayName("2. 제목에 키워드가 부분 일치해도 검색된다")
        void searchSeries_partialMatch_returnsMatchingSeries() {
            String accessToken = newUserToken();
            String keyword = uniqueSearchKeyword();
            Long seriesId = createSeries(accessToken, "접두" + keyword + "접미", "본문");
        ...
        void searchSeries_noMatch_returns200WithEmptyContent() {
        ...
        void searchSeries_pagingWithSizeOne_computesTotalPagesAndIsLast() {
            String accessToken = newUserToken();
            String keyword = uniqueSearchKeyword();
            Long olderId = createSeries(accessToken, keyword + " 첫번째", "본문");
            Long newerId = createSeries(accessToken, keyword + " 두번째", "본문");
    ...
    class GetUserSeriesList {

        @Test
        @DisplayName("1. 비로그인으로 조회해도 200과 해당 유저의 시리즈만 반환한다")
        void getUserSeriesList_anonymous_returnsOnlyThatUsersSeries() {
            LoginResponse userA = createUserAndLogin(client, uniqueEmail(), DEFAULT_PASSWORD, uniqueNickname());
            Long seriesA1 = createSeries(userA.accessToken(), "A의 유저별 목록 시리즈1", "본문");
            Long seriesA2 = createSeries(userA.accessToken(), "A의 유저별 목록 시리즈2", "본문");
        ...
        void getUserSeriesList_userWithNoSeries_returnsEmptyContent() {
            LoginResponse user = createUserAndLogin(client, uniqueEmail(), DEFAULT_PASSWORD, uniqueNickname());
        ...
        void getUserSeriesList_nonExistentUserId_returns200WithEmptyContent() {
        ...
        void getUserSeriesList_withdrawnUser_stillReturnsTheirSeries() {
            LoginResponse user = createUserAndLogin(client, uniqueEmail(), DEFAULT_PASSWORD, uniqueNickname());
            Long seriesId = createSeries(user.accessToken(), "탈퇴 전 만든 시리즈", "본문");
        ...
        @Test
        @DisplayName("5. soft delete된 시리즈는 목록에서 제외된다")
        void getUserSeriesList_excludesSoftDeletedSeries() {
            LoginResponse user = createUserAndLogin(client, uniqueEmail(), DEFAULT_PASSWORD, uniqueNickname());
            Long seriesId = createSeries(user.accessToken(), "유저별 목록에서 삭제될 시리즈", "본문");
    ...
    class GetMySeriesList {

        @Test
        @DisplayName("1. 미인증이면 401-1과 \"로그인 후 이용해주세요.\"를 반환한다")
        void getMySeriesList_unauthenticated_returns401_1() {
            expectEntryPointUnauthorized(client.get().uri("/api/series/me").exchange());
        }

        @Test
        @DisplayName("2. 성공하면 200과 내 시리즈만 포함된 목록을 반환한다")
        void getMySeriesList_success_returnsOnlyMySeries() {
            String accessToken = newUserToken();
            Long seriesId1 = createSeries(accessToken, "내 시리즈 목록1", "본문");
            Long seriesId2 = createSeries(accessToken, "내 시리즈 목록2", "본문");
        ...
        void getMySeriesList_excludesOtherUsersSeries() {
            String otherToken = newUserToken();
            Long otherSeriesId = createSeries(otherToken, "타인의 시리즈", "본문");

            String myToken = newUserToken();
        ...
        void getMySeriesList_noSeries_returnsEmptyContent() {
            String accessToken = newUserToken();
        ...
        @Test
        @DisplayName("5. soft delete된 시리즈는 목록에서 제외된다")
        void getMySeriesList_excludesSoftDeletedSeries() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "내 목록에서 삭제될 시리즈", "본문");
            deleteSeriesRequest(accessToken, seriesId).expectStatus().isOk();
    ...
    class UploadMedia {
        ...
        void uploadMedia_unauthenticated_returns401_1() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "미인증 썸네일 대상", "본문");

            RestTestClient.ResponseSpec response = client.post().uri("/api/series/" + seriesId + "/medias")
                    .contentType(MediaType.MULTIPART_FORM_DATA)
        ...
        void uploadMedia_success_returns201AndIsRetrievable() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "썸네일 생성 대상", "본문");
            ...
            assertThat(Files.exists(uploadedFilePath(uploadedUrlHolder[0]))).isTrue();
        ...
        void uploadMedia_reupload_replacesMediaAndDeletesOldFile() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "썸네일 교체 대상", "본문");

            SeriesMediaResponse first = uploadSeriesMedia(accessToken, seriesId, PNG_BYTES, "first.png");
            Path firstFilePath = uploadedFilePath(first.url());
        ...
        void uploadMedia_otherUsersSeries_returns403_1() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "남의 시리즈 썸네일 대상", "본문");

            String otherToken = newUserToken();

            expectResultCode(
                    uploadMediaRequest(otherToken, seriesId, mediaFilePart(PNG_BYTES, "thumbnail.png", MediaType.IMAGE_PNG)),
                    HttpStatus.FORBIDDEN, "403-1");
        }

        @Test
        @DisplayName("5. 존재하지 않는 시리즈면 404-5를 반환한다")
        void uploadMedia_nonExistentSeries_returns404_5() {
            String accessToken = newUserToken();

            expectResultCode(
                    uploadMediaRequest(accessToken, NON_EXISTENT_SERIES_ID, mediaFilePart(PNG_BYTES, "thumbnail.png", MediaType.IMAGE_PNG)),
                    HttpStatus.NOT_FOUND, "404-5");
        }

        @Test
        @DisplayName("6. 삭제된 시리즈면 404-5를 반환한다")
        void uploadMedia_softDeletedSeries_returns404_5() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "삭제 후 업로드할 시리즈", "본문");
            deleteSeriesRequest(accessToken, seriesId).expectStatus().isOk();
        ...
        void uploadMedia_emptyFile_returns400_4() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "빈 파일 업로드 대상", "본문");

            expectResultCode(
                    uploadMediaRequest(accessToken, seriesId, mediaFilePart(new byte[0], "empty.png", MediaType.IMAGE_PNG)),
        ...
        void uploadMedia_unsupportedFileType_returns415_1() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "지원 안 하는 형식 업로드 대상", "본문");
        ...
        void uploadMedia_missingPartContentType_returns415_1() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "Content-Type 없는 파일 업로드 대상", "본문");
        ...
        void uploadMedia_missingFilePart_returns500_1() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "file part 누락 대상", "본문");

            expectResultCode(
                    uploadMediaRequest(accessToken, seriesId, new LinkedMultiValueMap<>()),
    ...
    class GetMedia {
        ...
        void getMedia_anonymous_returns200WithMedia() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "비로그인 썸네일 조회 대상", "본문");
            SeriesMediaResponse uploaded = uploadSeriesMedia(accessToken, seriesId, PNG_BYTES, "thumbnail.png");
        ...
        void getMedia_otherUsersSeries_returns200() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "타인이 조회할 썸네일 대상", "본문");
            uploadSeriesMedia(ownerToken, seriesId, PNG_BYTES, "thumbnail.png");

            String otherToken = newUserToken();

            client.get().uri("/api/series/" + seriesId + "/medias")
                    .header("Authorization", bearer(otherToken))
                    .exchange()
                    .expectStatus().isOk()
                    .expectBody(seriesMediaBody())
        ...
        void getMedia_withoutMedia_returns200WithNullData() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "썸네일 없는 시리즈", "본문");
        ...
        void getMedia_nonExistentSeries_returns404_5() {
            expectResultCode(getMediaRequest(NON_EXISTENT_SERIES_ID), HttpStatus.NOT_FOUND, "404-5");
        }

        @Test
        @DisplayName("5. 삭제된 시리즈면 404-5를 반환한다")
        void getMedia_softDeletedSeries_returns404_5() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "삭제 후 썸네일 조회할 시리즈", "본문");
            deleteSeriesRequest(accessToken, seriesId).expectStatus().isOk();
    ...
    class DeleteMedia {
        ...
        void deleteMedia_unauthenticated_returns401_1() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "미인증 썸네일 삭제 대상", "본문");
        ...
        void deleteMedia_owner_returns200_andMediaAndFileAreGone() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "썸네일 삭제 대상", "본문");
            SeriesMediaResponse uploaded = uploadSeriesMedia(accessToken, seriesId, PNG_BYTES, "thumbnail.png");
            Path filePath = uploadedFilePath(uploaded.url());
            ...
            getMediaRequest(seriesId)
                    .expectStatus().isOk()
                    .expectBody(seriesMediaBody())
                    .value(body -> assertThat(body.data()).isNull());
            assertThat(Files.exists(filePath)).isFalse();
        }

        @Test
        @DisplayName("3. 타인의 시리즈 썸네일을 삭제하면 403-1을 반환한다")
        void deleteMedia_otherUsersSeries_returns403_1() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "남의 썸네일 삭제 대상", "본문");
            uploadSeriesMedia(ownerToken, seriesId, PNG_BYTES, "thumbnail.png");
        ...
        void deleteMedia_withoutMedia_returns404_7() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "썸네일 없이 삭제 시도할 시리즈", "본문");
        ...
        @Test
        @DisplayName("5. 존재하지 않는 시리즈면 404-5를 반환한다")
        void deleteMedia_nonExistentSeries_returns404_5() {
            String accessToken = newUserToken();
        ...
        @Test
        @DisplayName("6. 삭제된 시리즈면 404-5를 반환한다")
        void deleteMedia_softDeletedSeries_returns404_5() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "삭제 후 썸네일 삭제 시도할 시리즈", "본문");
            deleteSeriesRequest(accessToken, seriesId).expectStatus().isOk();
        ...
        void deleteMedia_alreadyDeleted_returns404_7() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "두 번 삭제할 썸네일 대상", "본문");
            uploadSeriesMedia(accessToken, seriesId, PNG_BYTES, "thumbnail.png");
    ...
    class AddPostToSeries {
        ...
        @Test
        @DisplayName("1. 미인증이면 401-1과 \"로그인 후 이용해주세요.\"를 반환한다")
        void addPostToSeries_unauthenticated_returns401_1() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "미인증 포스트 추가 대상 시리즈", "본문");
            Long postId = createPost(accessToken, "미인증 포스트 추가 대상 포스트");
        ...
        void addPostToSeries_success_returns200_1AndPostAppearsInSeries() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "포스트 추가 대상 시리즈", "본문");
            Long postId = createPost(accessToken, "추가될 포스트");
            ...
            getSeriesPostsRequest(seriesId)
                    ...
                    .value(body -> {
                        ...
                        assertThat(body.data().get(0).id()).isEqualTo(postId);
                        assertThat(body.data().get(0).seriesId()).isEqualTo(seriesId);
                    });
        }

        @Test
        @DisplayName("3. 같은 포스트를 두 번 추가해도 200-1이고 목록에 한 번만 담긴다 (멱등)")
        void addPostToSeries_duplicate_isIdempotent() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "중복 추가 대상 시리즈", "본문");
            Long postId = createPost(accessToken, "중복 추가될 포스트");
        ...
        void addPostToSeries_movesPostFromPreviousSeries() {
            String accessToken = newUserToken();
            Long oldSeriesId = createSeries(accessToken, "포스트를 잃을 시리즈", "본문");
            Long newSeriesId = createSeries(accessToken, "포스트를 얻을 시리즈", "본문");
            Long postId = createPostInSeries(accessToken, oldSeriesId, "이사 갈 포스트");
        ...
        void addPostToSeries_otherUsersPost_returns403_1() {
            String myToken = newUserToken();
            Long seriesId = createSeries(myToken, "내 시리즈", "본문");

            String otherToken = newUserToken();
            Long otherPostId = createPost(otherToken, "타인의 포스트");
        ...
        void addPostToSeries_otherUsersSeries_returns403_1() {
            String ownerToken = newUserToken();
            Long otherSeriesId = createSeries(ownerToken, "타인의 시리즈", "본문");

            String myToken = newUserToken();
            Long myPostId = createPost(myToken, "내 포스트");
        ...
        void addPostToSeries_nonExistentPostId_returns404_3() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "없는 포스트를 추가할 시리즈", "본문");
        ...
        void addPostToSeries_nonExistentSeriesId_returns404_5() {
            String accessToken = newUserToken();
            Long postId = createPost(accessToken, "없는 시리즈에 추가될 포스트");
        ...
        void addPostToSeries_softDeletedPost_returns404_3() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "삭제된 포스트를 추가할 시리즈", "본문");
            Long postId = createPost(accessToken, "삭제될 포스트");
        ...
        void addPostToSeries_softDeletedSeries_returns404_5() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "삭제 후 포스트를 추가할 시리즈", "본문");
            Long postId = createPost(accessToken, "삭제된 시리즈에 추가될 포스트");
    ...
    class RemovePostFromSeries {
        ...
        @Test
        @DisplayName("1. 미인증이면 401-1과 \"로그인 후 이용해주세요.\"를 반환한다")
        void removePostFromSeries_unauthenticated_returns401_1() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "미인증 포스트 제거 대상 시리즈", "본문");
            Long postId = createPostInSeries(accessToken, seriesId, "미인증 제거 대상 포스트");
        ...
        void removePostFromSeries_owner_returns200_1AndPostSurvives() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "포스트 제거 대상 시리즈", "본문");
            Long postId = createPostInSeries(accessToken, seriesId, "제거될 포스트");
        ...
        void removePostFromSeries_postWithoutSeries_returns404_5() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "관계 없는 포스트를 제거할 시리즈", "본문");
            Long postId = createPost(accessToken, "어느 시리즈에도 없는 포스트");
        ...
        void removePostFromSeries_mismatchedSeriesId_returns404_5() {
            String accessToken = newUserToken();
            Long realSeriesId = createSeries(accessToken, "포스트가 실제로 속한 시리즈", "본문");
            Long otherSeriesId = createSeries(accessToken, "엉뚱하게 넘길 시리즈", "본문");
            Long postId = createPostInSeries(accessToken, realSeriesId, "시리즈 불일치 확인용 포스트");
        ...
        void removePostFromSeries_otherUsersSeries_returns403_1() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "타인의 포스트 제거 대상 시리즈", "본문");
            Long postId = createPostInSeries(ownerToken, seriesId, "타인 시리즈의 포스트");
        ...
        void removePostFromSeries_nonExistentPostId_returns404_3() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "없는 포스트를 제거할 시리즈", "본문");
        ...
        void removePostFromSeries_softDeletedPost_returns404_3() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "삭제된 포스트를 제거할 시리즈", "본문");
            Long postId = createPostInSeries(accessToken, seriesId, "삭제 후 제거할 포스트");
        ...
        void removePostFromSeries_alreadyRemoved_returns404_5() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "두 번 제거할 시리즈", "본문");
            Long postId = createPostInSeries(accessToken, seriesId, "두 번 제거될 포스트");
    ...
    class GetSeriesPosts {

        @Test
        @DisplayName("1. 비로그인으로 조회해도 200-1과 시리즈의 모든 포스트 id가 반환된다 (순서 미단언)")
        void getSeriesPosts_anonymous_returns200WithAllPostIds() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "포스트 목록 조회 대상 시리즈", "본문");
            Long firstPostId = createPostInSeries(accessToken, seriesId, "목록 조회 포스트1");
            Long secondPostId = createPostInSeries(accessToken, seriesId, "목록 조회 포스트2");
        ...
        void getSeriesPosts_anonymous_exposesExactlyThirteenKeys() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "노출 키 확인용 시리즈", "본문");
            ...
            JsonNode post = firstSeriesPostNode(getSeriesPostsRequest(seriesId));
        ...
        void getSeriesPosts_anonymous_omitsUnexposedFields() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "비노출 필드 확인용 시리즈", "본문");
        ...
        void getSeriesPosts_emptySeries_returns200WithEmptyList() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "포스트 없는 시리즈", "본문");
        ...
        void getSeriesPosts_excludesSoftDeletedPost() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "포스트가 삭제될 시리즈", "본문");
            Long survivingPostId = createPostInSeries(accessToken, seriesId, "살아남을 포스트");
            Long deletedPostId = createPostInSeries(accessToken, seriesId, "삭제될 포스트");
        ...
        void getSeriesPosts_exposesPrivatePostToAnonymous() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "PRIVATE 포스트 시리즈", "본문");
            Long postId = createPost(ownerToken, "PRIVATE 포스트", "숨겨야 할 본문",
                    PublishStatus.PRIVATE, PostAccessLevel.FREE);
            ...
            getSeriesPostsRequest(seriesId)
                    ...
                    .value(body -> {
                        assertThat(body.data()).hasSize(1);
                        assertThat(body.data().get(0).id()).isEqualTo(postId);
                        assertThat(body.data().get(0).title()).isEqualTo("PRIVATE 포스트");
                        assertThat(body.data().get(0).publishStatus()).isEqualTo(PublishStatus.PRIVATE);
                    });
        ...
        void getSeriesPosts_exposesDraftPostToAnonymous() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "DRAFT 포스트 시리즈", "본문");
            Long postId = createPost(ownerToken, "DRAFT 포스트", "작성 중인 본문",
                    PublishStatus.DRAFT, PostAccessLevel.FREE);
            expectResultCode(addPostToSeriesRequest(ownerToken, seriesId, postId), HttpStatus.OK, "200-1");
            ...
            getSeriesPostsRequest(seriesId)
                    ...
                    .value(body -> {
                        assertThat(body.data()).hasSize(1);
                        assertThat(body.data().get(0).id()).isEqualTo(postId);
                        assertThat(body.data().get(0).publishStatus()).isEqualTo(PublishStatus.DRAFT);
                    });
        }

        @Test
        @DisplayName("8. PAID 포스트도 노출되고 accessLevel이 PAID로 나가며 body는 여전히 없다 (정상 사양)")
        void getSeriesPosts_exposesPaidPostWithoutBody() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "PAID 포스트 시리즈", "본문");
            Long postId = createPost(ownerToken, "PAID 포스트", "유료 본문",
                    PublishStatus.PUBLIC, PostAccessLevel.PAID);
            expectResultCode(addPostToSeriesRequest(ownerToken, seriesId, postId), HttpStatus.OK, "200-1");
            ...
            getSeriesPostsRequest(seriesId)
                    ...
                    .value(body -> {
                        assertThat(body.data()).hasSize(1);
                        assertThat(body.data().get(0).id()).isEqualTo(postId);
                        assertThat(body.data().get(0).accessLevel()).isEqualTo(PostAccessLevel.PAID);
                    });
        ...
        void getSeriesPosts_nonExistentSeries_returns200WithEmptyList() {
        ...
        void getSeriesPosts_softDeletedSeries_returns200WithEmptyList() {
            String accessToken = newUserToken();
            Long seriesId = createSeries(accessToken, "삭제 후 포스트 목록을 조회할 시리즈", "본문");
            Long postId = createPostInSeries(accessToken, seriesId, "시리즈와 함께 떨어져 나올 포스트");
        ...
        void getSeriesPosts_anonymous_isLikedAndIsBookmarkedAreAlwaysFalse() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "좋아요·북마크 확인용 시리즈(비로그인)", "본문");
            Long postId = createPostInSeries(ownerToken, seriesId, "좋아요·북마크될 포스트");
            ...
            getSeriesPostsRequest(seriesId)
                    ...
                    .value(body -> {
                        assertThat(body.data()).hasSize(1);
                        assertThat(body.data().get(0).isLiked()).isFalse();
                        assertThat(body.data().get(0).isBookmarked()).isFalse();
                        assertThat(body.data().get(0).likeCount()).isEqualTo(1L);
                        assertThat(body.data().get(0).bookmarkCount()).isEqualTo(1L);
                    });
        }

        @Test
        @DisplayName("12. 로그인 상태로 좋아요·북마크한 포스트를 조회하면 isLiked/isBookmarked가 true다")
        void getSeriesPosts_authenticatedWithLikeAndBookmark_returnsTrue() {
            String ownerToken = newUserToken();
            Long seriesId = createSeries(ownerToken, "좋아요·북마크 확인용 시리즈(로그인)", "본문");
            Long postId = createPostInSeries(ownerToken, seriesId, "내가 좋아요·북마크할 포스트");

            // 좋아요/북마크 전에는 로그인해도 false다.
            getSeriesPostsRequest(ownerToken, seriesId)
                    .expectStatus().isOk()
                    .expectBody(seriesPostsBody())
                    .value(body -> {
                        assertThat(body.data().get(0).isLiked()).isFalse();
            ...
            getSeriesPostsRequest(ownerToken, seriesId)
                    ...
                    .value(body -> {
                        ...
                        assertThat(body.data()).hasSize(1);
                        assertThat(body.data().get(0).id()).isEqualTo(postId);
                        assertThat(body.data().get(0).isLiked()).isTrue();
                        assertThat(body.data().get(0).isBookmarked()).isTrue();
                    });

            // 같은 포스트를 다른 유저가 보면 false다 — 개인화 값이 요청자 기준인지 확인한다.
            String otherToken = newUserToken();
            getSeriesPostsRequest(otherToken, seriesId)
                    .expectStatus().isOk()
                    .expectBody(seriesPostsBody())
                    .value(body -> {
                        assertThat(body.data().get(0).isLiked()).isFalse();
    ...
    class Scenarios {
        ...
        void scenario_createSeries_uploadThumbnail_addPosts_reflectedInUserSeriesList() {
            LoginResponse user = createUserAndLogin(client, uniqueEmail(), DEFAULT_PASSWORD, uniqueNickname());
            String accessToken = user.accessToken();
            Long userId = user.user().id();

            Long seriesId = createSeries(accessToken, "시나리오1 시리즈", "본문");

            SeriesListResponse justCreated = userSeriesListEntry(userId, seriesId);
            assertThat(justCreated.postCount()).isZero();
            assertThat(justCreated.thumbnailUrl()).isNull();

            SeriesMediaResponse thumbnail = uploadSeriesMedia(accessToken, seriesId, PNG_BYTES, "scenario1.png");

            SeriesListResponse withThumbnail = userSeriesListEntry(userId, seriesId);
            assertThat(withThumbnail.thumbnailUrl()).isEqualTo(thumbnail.url());
            assertThat(withThumbnail.postCount()).isZero();

            Long firstPostId = createPostInSeries(accessToken, seriesId, "시나리오1 포스트1");

            SeriesListResponse withOnePost = userSeriesListEntry(userId, seriesId);
            assertThat(withOnePost.postCount()).isEqualTo(1L);

            Long secondPostId = createPostInSeries(accessToken, seriesId, "시나리오1 포스트2");

            SeriesListResponse withTwoPosts = userSeriesListEntry(userId, seriesId);
        ...
        void scenario_deleteSeries_detachesPostsButKeepsThem() {
            LoginResponse user = createUserAndLogin(client, uniqueEmail(), DEFAULT_PASSWORD, uniqueNickname());
            String accessToken = user.accessToken();

            Long seriesId = createSeries(accessToken, "시나리오2 시리즈", "본문");
            Long postId = createPostInSeries(accessToken, seriesId, "시나리오2 포스트");

            getPostRequest(postId)
                    .expectStatus().isOk()
                    .expectBody(postBody())
                    .value(body -> assertThat(body.data().seriesId()).isEqualTo(seriesId));
            assertThat(seriesPostIds(seriesId)).containsExactly(postId);
            ...
            getUserSeriesListRequest(user.user().id(), "")
                    ...
                    .expectBody(seriesPageBody())
                    .value(body -> assertThat(body.data().content())
                            .extracting(SeriesListResponse::id).doesNotContain(seriesId));
        ...
        void scenario_twoUsersAreIsolated() {
            LoginResponse userA = createUserAndLogin(client, uniqueEmail(), DEFAULT_PASSWORD, uniqueNickname());
            LoginResponse userB = createUserAndLogin(client, uniqueEmail(), DEFAULT_PASSWORD, uniqueNickname());

            Long seriesOfA = createSeries(userA.accessToken(), "시나리오3 A의 시리즈", "A의 본문");
            ...
            getMySeriesListRequest(userA.accessToken(), "")
                    ...
                    .expectBody(seriesPageBody())
                    .value(body -> assertThat(body.data().content())
                            .extracting(SeriesListResponse::id).contains(seriesOfA));
            getMySeriesListRequest(userB.accessToken(), "")
                    .expectStatus().isOk()
                    .expectBody(seriesPageBody())
                    .value(body -> assertThat(body.data().content())
                            .extracting(SeriesListResponse::id).doesNotContain(seriesOfA));
        ...
        void scenario_search_read_update_searchAgain() {
            String accessToken = newUserToken();
            String oldKeyword = uniqueSearchKeyword();
            String newKeyword = uniqueSearchKeyword();

            Long seriesId = createSeries(accessToken, oldKeyword, "시나리오4 본문");

            searchSeriesRequest("?keyword=" + oldKeyword)
                    .expectStatus().isOk()
                    .expectBody(seriesPageBody())
                    .value(body -> assertThat(body.data().content())
                            .extracting(SeriesListResponse::id).containsExactly(seriesId));

            getSeries(seriesId)
                    .expectStatus().isOk()
                    .expectBody(seriesBody())
                    .value(body -> assertThat(body.data().title()).isEqualTo(oldKeyword));
        ...
        void scenario_replaceThumbnail_updatesThumbnailUrlInUserSeriesList() {
            LoginResponse user = createUserAndLogin(client, uniqueEmail(), DEFAULT_PASSWORD, uniqueNickname());
            String accessToken = user.accessToken();
            Long userId = user.user().id();

            Long seriesId = createSeries(accessToken, "시나리오5 시리즈", "본문");
            assertThat(userSeriesListEntry(userId, seriesId).thumbnailUrl()).isNull();

            SeriesMediaResponse first = uploadSeriesMedia(accessToken, seriesId, PNG_BYTES, "scenario5-first.png");
            ...
            SeriesMediaResponse second = uploadSeriesMedia(accessToken, seriesId, PNG_BYTES, "scenario5-second.png");
            assertThat(second.id()).isEqualTo(first.id());
            assertThat(second.url()).isNotEqualTo(first.url());

            SeriesListResponse afterReplace = userSeriesListEntry(userId, seriesId);
